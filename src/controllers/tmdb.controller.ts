...
> => {
  ...
  try {
    ...
    console.error("❌ Error in fetchTrendingMoviesFromAPI:", error);
...
const GetTrendingMovies = asyncHandler(async (req: Request, res: Response) => {
  try {
    ...
    if (cacheDoc.exists) {
      ...
      if (cacheAgeHours < 6) {
        console.log(
          `📋 Returning cached data (${cacheAgeHours.toFixed(1)} hours old)`
        );
        ...
        console.log(
          `⏰ Cache is ${cacheAgeHours.toFixed(1)} hours old, refreshing...`
        );
      }
    }

    console.log("🔄 Fetching fresh trending movies from IMDb...");
...
const fetchMovieFromIMDb = async (imdbId: string): Promise<any> => {
  ...
  try {
    ...
    console.log(`✅ Successfully scraped data for ${imdbId}`);
    return movieData;
  } catch (error) {
    console.error(`❌ Error scraping movie data for ${imdbId}:`, error);
    ...
    try {
      ...
      console.error("⚠️ Error closing scraper:", closeError);
    }
  }
};
...
const storeMovieDataInFirebase = async (
  ...
  try {
    ...
    console.error(`❌ Error storing movie data for ${imdbId}:`, error);
...
const updateVideoSources = async (imdbId: string, movieData: any) => {
  ...
  try {
    ...
    console.log(`✅ Updated video sources for ${imdbId}`);
...
const GetMovieData = asyncHandler(async (req: Request, res: Response) => {
  try {
    ...
    console.error(
      `❌ Error getting movie data for ${req.params.imdbId}:`,
      error
    );
...
const deleteCommentFromMovie = async (
  ...
  try {
    ...
    if (!commentDoc.exists) {
      console.log(`ℹ️ Comment ${commentId} not found in movie ${imdbId}`);
      return;
    }
...
const getSimilarMovies = asyncHandler(async (req: Request, res: Response) => {
  try {
    ...
    console.log(`🔍 Getting similar movies for: ${title}`);
    ...
    for (const movie of similarMovies) {
      try {
        ...
        console.warn(
          `⚠️ Failed to get details for similar movie: ${movie}`,
          error
        );
